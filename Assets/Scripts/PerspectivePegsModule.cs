...
public class PerspectivePegsModule : MonoBehaviour {
	...
	void Start() {
		Debug.Log("Perspective Pegs start!");
	...
	int CheckMatch(string pattern, string field) {
		...
		int outval;
		string outstr = "Matching " + pattern + " vs " + field + ", result is ";
	...
	string FillView(string part, string prime, bool isChosen, int view) {
		...
		// Debug.Log ("Trying to complete " + part + " vs " + prime + ", chosen is " + isChosen);
	...
	List<Peg> GenerateColours(int chosenColour, List<Permute> permutations) {
		...
		string output = ColoursToString (currentSequence);
		...
		/*output = "Pegs are ";
		for (int i = 0; i < 5; i++) {
			output += ColoursToString (Pegs [i].colours);
			if (i != 4) {
				output += ", ";
			}
		}
	...
	void OnActivate() {
		...
		string[] colourNames = new string[] { "RED", "YELLOW", "GREEN", "BLUE", "PURPLE" };
		...
		string serialNum = KMBombInfoExtensions.GetSerialNumber (BombInfo).ToLower();
		//string serialNum = "0abdc0";

		int batteryCount = KMBombInfoExtensions.GetBatteryCount (BombInfo);
		//int batteryCount = 1;

		// Determine chosen colour
		string output = "Determining first colour: ";
		...
		output += " (" + colourNames [chosenColour] + ")";
		...
		output = "Pegs are ";
		for (int i = 0; i < 5; i++) {
			output += ColoursToString (result [i].colours);
			if (i != 4) {
				output += ", ";
			}
		}
		...
		output = "Correct sequence is: ";
		for (int i = 0; i < 3; i++) {
			output += CorrectSequence [i];
		}
		if (IsPalindrome) {
			output += ", is palindromatic";
		}
		Debug.Log (output);
